...
function adaptive_header(w,h) {
		var header750=$('.header-menu-750');
		var headerContacts=$('.header-menu__list');
	if(w<768){
		if(!headerContacts.hasClass('done')){
			headerContacts.addClass('done').appendTo(header750);
		}
	}else{
		if(headerContacts.hasClass('done')){
			headerContacts.removeClass('done').prependTo($('.header-menu'));
		}
	}

	if(w<768){
		if(!$('.header__search').hasClass('done')){
			$('.header__search').addClass('done').appendTo(header750);
		}
	}else{
		if($('.header__search').hasClass('done')){
			$('.header__search').removeClass('done').prependTo($('.header__column_3'));
		}
	}
}
...
$('.header-menu__icon').click(function(event) {
	$(this).toggleClass('active');
	$('.header-menu-750').toggleClass('active');
	if($(this).hasClass('active')){
